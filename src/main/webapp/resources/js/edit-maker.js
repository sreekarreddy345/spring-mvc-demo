...
var EditMaker = function(_containerID,_loadSettings,_saveSettings) {
    ...
    this.getIndexFromCache = function(url){
        if(this.loadSettings.disableCache) return false;
        var index = false;
    ...
    this.save = function() {
        ...
        .done(function(data) {
            if(data) _this.loadIntoUI(data);
            else _this.loadIntoUI(formData);
    ...
    this.getFormData = function() {
        ...
        for(var i=0; i<fields.length; ++i) {
            ...
            if(field.type == "checkbox") {
                tosend[field.name] = (field.checked)?1:0;
            } else {
    ...
    this.updateCache = function(item) {
        if(this.loadSettings.disableCache || !item.url) return false;
